...
Page({
    ...
    updateTaskProgress(taskId) {
        const tasks = this.data.tasks.map((task) => {
            if (task.id === taskId) {
                ...
                return Object.assign(Object.assign({}, task), { completedPomos, progress, status: progress >= 100 ? 'completed' : task.status });
            }
            return task;
        });
        this.setData({ tasks });
    ...
    reportTimerComplete(_duration, _isInterrupted) { /* TODO: 调用后端API */ },
    ...
    saveTask() {
        ...
        if (this.data.isEditing && id) {
            tasks = tasks.map(task =>
                task.id === id
                    ? Object.assign(Object.assign({}, task), { title, estimatedPomos, priority, priorityText, duration, deadline: this.data.taskForm.deadline || undefined, description: this.data.taskForm.description || undefined })
                    : task
            );
            this.setData({ tasks });
            this.saveTasks();
