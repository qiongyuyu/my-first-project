...
Page({
    ...
    showTaskModal(e) {
        if (e && e.currentTarget.dataset.id) {
            ...
            this.setData({
                taskForm: { id: '', title: '', estimatedPomoIndex: 1, priorityIndex: 1, durationIndex: 0 },
                isEditing: false
            });
        }
    ...
    saveTask() {
        ...
        if (this.data.isEditing && id) {
            tasks = tasks.map(task =>
                task.id === id
                    ? Object.assign(Object.assign({}, task), { title, estimatedPomos, priority, priorityText, duration })
                    : task
            );
            this.setData({ tasks });
            this.saveTasks();
