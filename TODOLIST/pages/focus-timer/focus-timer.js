...
Page({
    ...
    updateTaskProgress(taskId) {
        const tasks = this.data.tasks.map((task) => {
            if (task.id === taskId) {
                ...
                return Object.assign(Object.assign({}, task), { completedPomos,
                    progress, status: progress >= 100 ? 'completed' : task.status });
            }
            return task;
        });
        this.setData({ tasks });
    ...
    saveTask() {
        ...
        if (this.data.isEditing && id) {
            // 更新任务
            tasks = tasks.map(task => task.id === id ? Object.assign(Object.assign({}, task), { title,
                description,
                estimatedPomos,
                priority,
                priorityText }) : task);
