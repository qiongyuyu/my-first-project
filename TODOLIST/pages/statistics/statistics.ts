...
Page({
  ...
  loadStatistics() {
    ...
    // 计算总专注时间（小时）
    const totalSeconds = records.reduce((sum: number, record: any) => sum + (record.duration || 0), 0);
    const totalHours = (totalSeconds / 3600).toFixed(1);

    // 计算总番茄数
    const totalPomos = records.filter((r: any) => !r.isInterrupted).length;
  ...
  initCharts() {
    // 获取图表组件
    this.selectComponent('#duration-chart').init((canvas: any, width: number, height: number) => {
    ...
    this.selectComponent('#category-chart').init((canvas: any, width: number, height: number) => {
      const chart = echarts.init(canvas, null, {
        width,
        height
      });
   ...
   * 设置专注时长图表选项
   */
  setDurationChartOption(chart: any) {
    // 模拟数据
    const data = [
   ...
   * 设置任务类别图表选项
   */
  setCategoryChartOption(chart: any) {
    // 模拟数据
    const data = [
      { name: '学习', value: 40 },
      { name: '工作', value: 30 },
