...
Page({
  ...
  generateAIPlan() {
    ...
    // 根据分数分配到四象限
    const quadrants = this.data.quadrants.map(() => [] as any[]);
    ...
    // 更新数据
    this.setData({
      quadrants: this.data.quadrants.map((quadrant: any, index: number) => ({
        ...quadrant,
        tasks: quadrants[index]
      }))
    });
  },
  ...
  generateManualPlan() {
    ...
    tasks.forEach((task: any) => {
      ...
      quadrants[quadrantIndex].push({
        ...task,
        urgency: task.priority === 'high' ? 80 : task.priority === 'medium' ? 50 : 30,
        importance: task.priority === 'high' ? 90 : task.priority === 'medium' ? 70 : 40,
        x: Math.random() * 150,
        y: Math.random() * 150,
        dragging: false
      });
   ...
   * 重要性权重变化
   */
  onImportanceWeightChange(e: any) {
    this.setData({
   ...
   * 获取象限的矩形区域
   */
  getQuadrantRect(_quadrantIndex: number) {
    // 实际应该通过选择器获取元素位置
    // 简化返回固定值
    return {
      left: 0,
